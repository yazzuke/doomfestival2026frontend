...
import { AllDjsModal } from "../ui/AllDjModals";
...
export const LineUpEventSection = ({ artists, description }: LineUpEventSectionProps) => {
  ...
  // Mostrar solo los primeros 4 DJs destacados
  const featuredArtists = artists.slice(0, 4);
  const hasMoreArtists = artists.length > 4;
      ...
      <section className="relative py-20 bg-black overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          ...
          >
            ...
            <motion.h2 
              className="text-4xl md:text-7xl font-black text-white mb-4 md:mb-6 tracking-wider relative"
              animate={{
                textShadow: [
            ...
            {description && (
              <motion.p 
                className="text-[#84cc16] text-base md:text-lg max-w-2xl mx-auto tracking-wide px-4"
              ...
              >
                ...
                <div className="relative h-64 md:h-80 overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 md:p-6">
                    <motion.h3 
                      className="font-rajdhani text-lg md:text-3xl font-black text-white mb-2 group-hover:text-[#84cc16] transition-colors tracking-wider"
                      whileHover={{
                ...
                <div className="absolute top-0 left-0 w-6 h-6 md:w-8 md:h-8 border-t-2 border-l-2 border-[#84cc16] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="absolute top-0 right-0 w-6 h-6 md:w-8 md:h-8 border-t-2 border-r-2 border-[#84cc16] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="absolute bottom-0 left-0 w-6 h-6 md:w-8 md:h-8 border-b-2 border-l-2 border-[#84cc16] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="absolute bottom-0 right-0 w-6 h-6 md:w-8 md:h-8 border-b-2 border-r-2 border-[#84cc16] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </motion.div>
            ))}
          </div>

          {/* Botón para ver todos los DJs */}
          {hasMoreArtists && (
            <motion.div 
            ...
            >
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="group relative px-6 md:px-10 py-4 md:py-5 bg-[#84cc16] text-black font-black text-lg md:text-xl tracking-wider overflow-hidden transition-all duration-300 hover:bg-white"
              >
                <span className="relative z-10 font-orbitron">
                  {t('lineup.viewAll')} ({artists.length})
                </span>
                ...
                {/* Bordes decorativos */}
                <div className="absolute top-0 left-0 w-2 h-2 md:w-3 md:h-3 border-t-2 border-l-2 border-black" />
                <div className="absolute top-0 right-0 w-2 h-2 md:w-3 md:h-3 border-t-2 border-r-2 border-black" />
                <div className="absolute bottom-0 left-0 w-2 h-2 md:w-3 md:h-3 border-b-2 border-l-2 border-black" />
                <div className="absolute bottom-0 right-0 w-2 h-2 md:w-3 md:h-3 border-b-2 border-r-2 border-black" />
              </motion.button>
            </motion.div>
          )}
        </div>
